...
authDomain: "https://www.google.com/search?q=aahar-nidra.firebaseapp.com",
...
if (!apiKey) return "Error: No API Key. Add REACT_APP_GEMINI_API_KEY to Vercel Settings.";
...
const width = 320; const height = 180; const padding = { top: 20, bottom: 30, left: 30, right: 10 };
const chartHeight = height - padding.top - padding.bottom; const chartWidth = width - padding.left - padding.right;
const barWidth = 20; const gap = (chartWidth - (data.length * barWidth)) / (data.length + 1);
...
return <g key={h}><line x1={padding.left} y1={y} x2={width - padding.right} y2={y} stroke="#e5e7eb" strokeWidth="1" strokeDasharray="4 4"/><text x={padding.left - 5} y={y + 3} fontSize="10" fill="#9ca3af" textAnchor="end">{h}h</text></g>;
})}
{data.map((d, i) => {
...
const [selectedMealTime, setSelectedMealTime] = useState(getCurrentMealTime()); // Auto-set time on load
...
// Update selected time whenever tab changes to food to keep it accurate
useEffect(() => {
...
mealTime: selectedMealTime, // Saving the selected time slot
